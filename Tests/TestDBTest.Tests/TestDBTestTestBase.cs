...
namespace TestDBTest.Tests
{
    public abstract class TestDBTestTestBase : AbpIntegratedTestBase<TestDBTestTestModule>
    {
        ...
        protected void UsingDbContext(Action<TestDBTestDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<TestDBTestDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<TestDBTestDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<TestDBTestDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<TestDBTestDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<TestDBTestDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<TestDBTestDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<TestDBTestDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<TestDBTestDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<TestDBTestDbContext, Task<T>> func)
        {
            T result;
